...
class ArtificialNeuralNetwork(object):
    def __init__(self):
        ...
        self.hidden_units = 6 #Number of nodes in hidden layer
    ...
    def train(self, X, y, iteration,activation):
        for i in range(iteration):
            y_hat = self._forward_pass(self, X,activation)
...
if __name__=='__main__':
    ...
    iteration = 100
    activation = "sigmoid"
    model.train(ArtificialNeuralNetwork,train_X, train_y, iteration, activation) # train model
    pre_y = model.predict(test_X,activation) # predict
    
    ax = plt.axes(projection='3d')
    xline = T_X[:100,0]
    yline = np.array(T_X[:100,2])
    zline = pre_y[:100]
  
    zline2 = np.array(TT_y[:100])
